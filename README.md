...
**Read-only role-based operations dashboards for OpenClaw runtimes**
...
- 把产品定位从“资源目录台”推进到“角色化经营观察台”
- 当前这轮重点是 **页面可信度收口**：空态产品化、proxy 指标口径说明、角色语言统一

### 事实状态表 / Fact Status

| 能力 / Capability | 当前状态 / Status | 评审口径 / Review framing |
|---|---|---|
| 角色化只读工作台 (`Overview / Operations / Adoption / Outcomes / Governance / Evidence`) | 已实现 | 当前主体验收面 |
| `Evidence -> Finding -> Recommendation` 治理链路 | 已实现 | 可支撑“发现 -> 证据 -> 人工复核”的只读闭环 |
| Gateway runtime-plane 读取 (`operator.read`) | 已实现 | 只读读取，不包含任何控制写操作 |
| `SIDECAR_TARGETS_FILE` 多目标注册 | 已实现 | 依赖本地静态 registry，不包含 service discovery |
| Filesystem + mock 双模式 | 已实现 | mock-first，filesystem 为当前真实 adapter |
| 指标可信度展示 (`proxy / observational / limited sample`) | 已实现 | 当前版本强调避免把观察指标误读成经营事实 |
| Web 实时推送渲染 | 试验中 | sidecar 会被事件触发刷新，但 web 当前仍以 polling 渲染 |
| 动态 target discovery | 明确未做 | 当前只支持本地 registry，可后续并行探索 |
| 写回、ack/snooze、owner、dueDate、RBAC、chat/control | 明确不做 | 当前版本严格保持只读边界 |

当前对外评审统一口径：

- **是什么**：OpenClaw 外部只读治理与运营观察台
- **不是什么**：不是控制面、不是写回平台、不是聊天前端
- **当前到哪一步**：`v0.3 alpha`，已完成治理链路与角色化 IA，正在补“指标可信度”和评审叙事收口

### 界面预览 / UI Preview

![Overview role-based dashboard](docs/screenshots/overview-role-dashboard.png)
![Operations workbench](docs/screenshots/operations-workbench.png)
![Adoption workbench](docs/screenshots/adoption-dashboard.png)
![Governance workbench](docs/screenshots/governance-dashboard.png)

当前截图展示的是新的角色化首页与工作台：

- `Overview` 先回答四件事：稳不稳、有没有人在用、有没有成效信号、有没有治理风险
- `Operations` 先给稳定性判断和异常热点，没有数据时用解释型空态明确说明“当前未观察到热点”
- `Adoption` 先看活跃、趋势、Top workspace / agent 与复用 proxy，并明确哪些数字只是观察型口径
- `Governance` 先看风险态势、finding 摘要与证据回链；`Evidence` 继续承担治理闭环里的证据桥接

配套说明文档：

- [Role-Based IA](docs/product/role-based-ia.md)
- [Metric Definitions](docs/product/metric-definitions.md)
- [Roadmap v0.3](docs/roadmap-v0.3.md)
- [Known Limitations](docs/v0.3-known-limitations.md)

### 当前已经落地的能力

#### 角色工作台 (Role Workbenches)
- `Overview`：经营观察首页，优先展示 Stability / Adoption / Outcome / Risk 四个判断入口
- `Operations`：运维工作台，聚合 health score、错误趋势、异常热点与配置健康
- `Adoption`：运营工作台，聚合活跃 proxy、趋势、Top workspace / agent 与 retention proxy
- `Outcomes`：推广观察工作台，用更克制的方式汇总团队覆盖、重复使用与推广阻碍
- `Governance`：治理工作台，汇总风险态势、compliance gap、findings brief 与建议优先级
...
我们推荐按以下顺序通过控制台进行功能演示，以完整体验“从判断到证据”的闭环：

1.  **Overview (`/`)**：先看 Stability / Adoption / Outcome / Risk 四张总卡与今天最值得关注的三件事。
2.  **Operations (`/operations`)**：看 health score、错误趋势、节点/Cron 异常与配置健康。
3.  **Adoption (`/adoption`)**：看活跃 proxy、趋势、Top workspace / agent 和 retention proxy。
4.  **Governance (`/governance`) → Findings (`/findings/:id`) → Evidence (`/evidence/:id`)**：沿着风险态势、发现摘要和证据链完成只读排查。
