...
当前截图展示的是接入本地 OpenClaw runtime 之后的 `Overview` 页面：左侧为语言与数据源标识，右侧为治理总览卡片、目标集群与风险汇总。
...
| Gateway runtime plane | `OPENCLAW_GATEWAY_URL` 指向的 Gateway WebSocket，以及 `openclaw.json` 中的 `gateway.auth.token` | 仅用 `role=operator` + `scopes=[operator.read]` 读取 `system-presence`、`node.list`、`sessions.list`、`tools.catalog`，补充 presence / nodes / tools / plugins，并在可用时优先使用 runtime sessions；shared token 会优先从 `openclaw.json` 自动解析 |
