...
function buildChips(
  ...
  return [
    ...
    `${t("runtime.gateway")}: ${runtime.gateway.dataReaderHealth === "healthy" || runtime.gateway.dataReaderHealth === "partial" ? t("status.ok") : translateStatus(runtime.gateway.connectionState)}`,
    ...
    <section className={`runtime-status-bar${error ? " runtime-status-bar-warning" : ""}`}>
      <div className="runtime-status-bar-header">
        ...
        <span className={`runtime-pill runtime-pill-${runtime ? (runtime.gateway.dataReaderHealth === "healthy" || runtime.gateway.dataReaderHealth === "partial" ? "connected" : runtime.gateway.connectionState) : "unknown"}`}>
          {runtime ? `${t("runtime.gateway")}: ${runtime.gateway.dataReaderHealth === "healthy" || runtime.gateway.dataReaderHealth === "partial" ? t("status.ok") : translateStatus(runtime.gateway.connectionState)}` : t("runtime.loading")}
        </span>
      </div>
