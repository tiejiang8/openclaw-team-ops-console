...
test("filesystem adapter activates for sidecar and official OpenClaw read-only envs", () => {
  ...
  assert.equal(hasFilesystemAdapterConfiguration({ OPENCLAW_GATEWAY_URL: "ws://127.0.0.1:4318/gateway" }), true);
  assert.equal(hasFilesystemAdapterConfiguration({ OPENCLAW_STATE_DIR: "/path/to/your/.openclaw" }), false);
  ...
  assert.equal(createSidecarAdapterFromEnv({ OPENCLAW_GATEWAY_URL: "ws://127.0.0.1:4318/gateway" }).mode, "external-readonly");
  ...
  assert.equal(createSidecarAdapterFromEnv({ OPENCLAW_STATE_DIR: "/path/to/your/.openclaw" }).mode, "mock");
});
